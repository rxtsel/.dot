...
let
  ...
  # Your swaync config (same behavior, only formatted via toJSON).
  swayncConfig = {
...
in
{
  ...
  xdg.configFile."swaync/config.json".text = builtins.toJSON swayncConfig;
  xdg.configFile."swaync/style.css".text = swayncStyle;

  # xdg.configFile."swaync/icons/whatever.svg".source = ./icons/whatever.svg;
}
